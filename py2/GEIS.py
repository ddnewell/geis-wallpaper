...
class plot:
            ...
            try:
                with open(clockFile) as cf:
                    ...
                    for i in range(len(sortCities)):
                        ...
                        x = xPos/self.map.xmax
                        y = yPos/self.map.ymax
                        # Adjust x for spread - TODO: determine why spread occurs
                        x -= (x-0.5)*0.013
    ...
    def setBlueMarbleBackground(self):
        ...
        self.map.bluemarble(scale=self.cfg["mapScale"])
    ...
    def setWallpaper(self, cleanup=True):
        ...
        if self.saved:
            ...
            w, h = bmpImage.size
            wDiff = int((w-self.cfg["screenSize"][0])*self.cfg["widthAdj"])
            hDiff = int((h-self.cfg["screenSize"][1])*self.cfg["heightAdj"])
