...
class terminator:
    ...
    def setPrevDec(self):
        ...
        self.prev = datetime.datetime(year, 12, 31, tzinfo=pytz.utc)
    ...
    def setMidnight(self):
        ...
        day = self.utcNow.day
        self.midnight = datetime.datetime(year, month, day, tzinfo=pytz.utc)
    ...
    def updateConstants(self):
        ...
        self.hours = self.TimeDiff.seconds/60/60
        
        self.mu = -3.6 + 0.9856 * self.days
        self.phi = self.mu + 1.9 * math.sin(math.radians(self.mu))
        self.omicron = self.phi + 102.9
        self.delta = 22.8 * math.sin(math.radians(self.omicron)) + 0.6 * math.pow(math.sin(math.radians(self.omicron)), 3)
        
        self.sigma = -15 * self.hours
...
class irradiation:
    ...
    def calcDaylightMesh(self,
                        lonInterval=10., lonRange=360., latInterval=10., latRange=160.,
                        lonOffset=0., latOffset=0., fast=True, arrayGrid=True):
        ''' Calculate Irradiation mesh '''
        # Generate mesh grid
